...
class S2TDataset(Dataset):
    default_keywords = ["up", "down", "left", "right", "yes", "no", "on", "off", "go", "stop", "none"]
    ...
    def __init__(self, dataset, args, keywords=None):
        ...
        self.keyword_ids = np.array(list(self.tokenizer(keyword)["input_ids"][0] for keyword in keywords[:-1]) + [-1])
    ...
    def collate_fn(self, pre_batch):
        ...
        keywords_tensor[(np.sum(keywords_tensor,axis=1) == 0),-1]=1

        s = []
        at = []
        for i in range(len(sent_in)):
            if np.sum(keywords_tensor,axis=1)[i] == 1:
                s.append(sent_in[i])
                at.append(audio_tensor[i])

        sent_in = s
        audio_tensor = torch.stack(at)

        keywords_tensor = keywords_tensor[(np.sum(keywords_tensor,axis=1) == 1),:]

        bs = len(audio_tensor)

        padding_id = self.tokenizer(text="<pad>")["input_ids"][0]

        cutoffs = [len(x)-1 for x in sent_in]#[np.random.randint(0, len(x) - 1) for x in sent_in]
        input_sents = []
        label_sents = []
